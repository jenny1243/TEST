...
<div class="x-body">
        ...
        <xblock>
            <div class="layui-input-inline layui-show-xs-block">
                <button class="layui-btn layui-btn-radius"
                        onclick="xadmin.open('添加商品种类','${pageContext.request.contextPath}/goodstype/addPage',700,400)">
                    <i
            ...
            <div class="layui-input-inline layui-show-xs-block">
                <a href="/goodstype/toGoodstypeExcel">
    ...
    <table class="layui-table layui-form">
        ...
        <c:forEach items="${list}" var="goodstype">
            <tr>
                <td>
                    <input type="checkbox" name="id" value="${goodstype.id}" lay-skin="primary">
                </td>
                <td>${goodstype.id}</td>
                ...
                <td class="td-manage">
                    ...
                    <a title="编辑"
                       onclick="xadmin.open('编辑','${pageContext.request.contextPath}/goodstype/editPage?id=${goodstype.id}',700,400)">
                        <i class="layui-icon">&#xe642;</i>
                    </a>

                    <a title="删除" onclick="member_del(this,'${goodstype.id}')">
                        <i class="layui-icon">&#xe640;</i>
                    </a>
...
<script>
    ...
    function member_del(obj, id) {
        layer.confirm('是否确认', {icon: 5}, function (index) {
            $.ajax({
                type: "post",
                url: "${pageContext.request.contextPath}/goodstype/delGoodstype",
                data: {id: id},
                dataType: "json",
                success: function (data) {
                    if(data.type == "success"){
                        alert(data.msg);
                        setTimeout(function () {
                            location.href= '/goodstype/goodstypeShow'
                        },1000)
                    }else {
                        alert(data.msg);
                    }
    ...
    function delAll(argument) {
        ...
        layer.confirm('确认要删除吗？', function (index) {
            ...
            $.ajax({
                type: "post",
                url: "${pageContext.request.contextPath}/goodstype/delmanyGoodstype",
                data: {ids: ids},
                dataType: "json",
                success: function (data) {
                    if(data.type == "success"){
